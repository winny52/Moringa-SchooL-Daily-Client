...
# Endpoint to add comment for a specific content item
...
def add_to_wishlist(content_id):
    ...
    user = db.session.get(User, user_id)  # Get the current user
    article = db.session.get(Content, content_id)  # Get the article by content ID
